...
// $_FILES['image']['tmp_name']を$uploaded_pathに一時保存
  ...
  // 一時保存するファイル名を代入
    $uploaded_path2 = "./css/".$filename2;
  // $_FILES['image']['tmp_name']を$uploaded_pathに一時保存
    $result2 = move_uploaded_file($_FILES['image2']['tmp_name'],$uploaded_path2);
    // $resultが存在するなら
    if($result2)
    {
      // アップロード
    $MSG2 = 'アップロード成功！ファイル名：'.$filename2;
    $img_path2 = $uploaded_path2;
    }
    // 存在しないならアップロード失敗
    else
    {
    $MSG2 = 'アップロード失敗！エラーコード：'.$_FILES['image']['error'];
    }
  }
    else
    {
...
?>
 ...
 <form action="keijiban3.php" method = "POST">
